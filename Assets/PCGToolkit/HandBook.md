...
> **延迟同步**：`FloatField`、`IntegerField`、`TextField`、`Vector3Field` 的值变更会在停止输入 **500ms** 后才同步到节点并触发图更新（防抖机制，避免频繁触发重计算）。`Toggle`、`ColorField`、`PopupField`、`Slider` 等瞬时控件保持即时同步。
...
| `MergeNode` | 合并多个几何体（最多 10 个输入端口：input0 ~ input9） |
| `DeleteNode` | 按条件删除点/面 |
...
- **Inject to Scene**：Output 节点 Inspector 中的 "Inject to Scene" 按钮，将执行结果临时实例化为场景 GameObject（`HideFlags.DontSave`，不随场景保存）。默认使用 PCG 官方灰调 PBR 材质渲染；若几何体带有 `@material` 属性且路径有效，则使用该属性指定的材质。 [0-cite-24](#0-cite-24)
...
PCG Toolkit 内置一个 Built-in Render Pipeline 的灰调 PBR 展示 Shader，用于节点预览窗口和 Live 模式场景注入的默认渲染。

### Shader 信息

| 属性 | 值 |
|---|---|
| Shader 路径 | `Hidden/PCG/SimplePBR` |
| 文件位置 | `Assets/PCGToolkit/Editor/Resources/Shaders/PCGSimplePBR.shader` |
| 渲染管线 | Built-in Render Pipeline（Surface Shader） |
| 视觉风格 | 灰调 PBR（类似 ZBrush 灰模），纯白 BaseColor，法线光照 + Fresnel AO 近似 |
| 暴露参数 | `Smoothness`（默认 0.35）、`Metallic`（默认 0）、`OcclusionStrength`（默认 0.6） |

### 加载方式

通过 `PCGDefaultMaterials.GetDefaultMaterial()` 获取材质实例（单例缓存）。

```
加载 fallback 链:
Hidden/PCG/SimplePBR → Standard → Diffuse → Default-Diffuse.mat
```

### 使用场景

- **节点 3D 预览窗口**（`PCGNodePreviewWindow`）：双击节点打开的预览窗口使用此材质
- **Inject to Scene**（`PCGScenePreview.InjectToScene`）：默认使用此材质，`@material` 属性有效时优先使用指定材质
- `PCGGeometryToMesh` 输出阶段不受影响，使用节点图中的材质分配逻辑
